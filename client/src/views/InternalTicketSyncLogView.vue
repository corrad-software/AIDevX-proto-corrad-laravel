...
import { Ticket, ChevronLeft, ChevronRight, Loader2, X } from "lucide-vue-next";

import AdminLayout from "@/layouts/AdminLayout.vue";
import * as BRANDING from "@/config/branding";
import { listInternalTicketSyncLogs, syncInternalTickets } from "@/api/cms";
import { useAuthStore } from "@/stores/auth";
import { useToast } from "@/composables/useToast";
import type { InternalTicketSyncLog } from "@/types";

const toast = useToast();
const auth = useAuthStore();

const canSync = computed(() => auth.user?.permissions?.includes("knowledge.manage") ?? false);
const logs = ref<InternalTicketSyncLog[]>([]);
...
const total = ref(0);
const limit = 20;
const selectedLog = ref<InternalTicketSyncLog | null>(null);
...
async function load() {
  loading.value = true;
  try {
    const res = await listInternalTicketSyncLogs(buildParams());
    logs.value = res.data;
    const meta = res.meta || {};
...
function selectLog(log: InternalTicketSyncLog) {
    ...
    <div class="flex h-[calc(100vh-4rem)] gap-4 p-4">
      <div class="w-80 flex-shrink-0 flex flex-col rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <div class="flex items-center gap-2">
            <Ticket class="h-4 w-4 text-teal-600" />
            <div>
              <h2 class="text-sm font-semibold text-slate-900">Log sync tiket dalaman</h2>
              <p class="text-[10px] text-slate-500">{{ BRANDING.ERP_SYSTEM_NAME }} / {{ BRANDING.PLATFORM_HEADER }} → Vector Store</p>
            </div>
          </div>
          <button
            v-if="canSync"
            class="flex items-center gap-1.5 rounded-lg bg-teal-700 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-teal-800 disabled:opacity-50"
            :disabled="syncing"
            @click="runSync"
          >
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-2">
          <div v-if="loading" class="py-8 text-center text-xs text-slate-400">Memuatkan...</div>
          ...
          <button
            ...
            :key="log.id"
            type="button"
            class="w-full text-left rounded-lg border border-slate-200 bg-white p-3 shadow-sm transition-all hover:border-slate-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-teal-400 focus:ring-offset-1"
            :class="selectedLog?.id === log.id ? 'border-teal-400 ring-2 ring-teal-200' : ''"
            @click="selectLog(log)"
          >
      ...
      >
        <div v-if="!selectedLog" class="flex flex-1 flex-col items-center justify-center text-slate-400">
          <Ticket class="h-12 w-12 mb-3 opacity-40" />
        ...
        <template v-else>
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4">
            <div class="grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
              ...
              <div>
                <p class="text-xs font-medium text-slate-500">Dicetuskan</p>
                <p class="mt-0.5">
              ...
              <div>
                <p class="text-xs font-medium text-slate-500">Pengguna</p>
              ...
              <div>
                <p class="text-xs font-medium text-slate-500">Tiket</p>
              ...
              <div>
                <p class="text-xs font-medium text-slate-500">Muat naik</p>
                <p class="mt-0.5 font-mono text-emerald-600">{{ selectedLog.uploaded }}</p>
              </div>
              <div>
                <p class="text-xs font-medium text-slate-500">Gagal</p>
                <p class="mt-0.5 font-mono text-rose-600">{{ selectedLog.failed }}</p>
              </div>
            </div>

            <div v-if="selectedLog.uploadedModules?.length">
              <p class="text-xs font-medium text-slate-500 mb-2">Modul dimuat naik</p>
              <ul class="rounded-lg border border-slate-200 divide-y divide-slate-100">
                ...
                >
                  <div class="flex items-center gap-2">
                    <span class="w-6 h-6 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center text-xs font-medium">
                      {{ i + 1 }}
                    </span>
                    {{ mod }}
                  </div>
            ...
            <div>
              <p class="text-xs font-medium text-slate-500 mb-2">Senarai tiket</p>
              <div
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <p v-if="t.assignedAgent"><span class="font-medium text-slate-500">Ejen:</span> {{ t.assignedAgent }}</p>
                    <p v-if="t.status"><span class="font-medium text-slate-500">Status:</span> {{ t.status }}</p>
                    <p v-if="t.type"><span class="font-medium text-slate-500">Jenis:</span> {{ t.type }}</p>
                    <p v-if="t.priority"><span class="font-medium text-slate-500">Keutamaan:</span> {{ t.priority }}</p>
                    <p v-if="t.contactName"><span class="font-medium text-slate-500">Pemohon:</span> {{ t.contactName }}</p>
                    <p v-if="t.companyName"><span class="font-medium text-slate-500">Pelanggan:</span> {{ t.companyName }}</p>
              ...
              >
                {{ selectedLog.totalTickets }} tiket disegerakkan. Jalankan <strong>Sync Now</strong> untuk rekod dengan butiran penuh.
              </div>
              <div v-else class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-500">
                Tiada tiket dalam sync ini.
              </div>
            </div>

            <div>
              <p class="text-xs font-medium text-slate-500">Mesej</p>
